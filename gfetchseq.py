...
if in_file[-9:] != '.interval':
    logger.info(f'{colored("Not a valid gatk_interval file", "red")}')
...
# loading Galaxy genome list
with open('galaxy_dbkeys.csv', 'rt') as f:
    reader = csv.reader(f)
    dbkeys = list(reader)[0]

# check if genome arg is in list
...
with open(in_file, 'rt') as f:
    ...
    for line in content:
        ...
        if index == 3:
            break
        index += 1
if len(records_list[1]) < 5:
    logger.info(f"{colored('Invalid content in .interval file', 'red')}")
    logger.info(f'Must have 5 columns: {colored("genName  genStart	genEnd	Strand	repName", "yellow")}')
    exit(0)

test_line = [records_list[1][0], records_list[1][1], records_list[1][2], records_list[1][4], 0, records_list[1][3]]
if (len(records_list[1][1]) == 0):
    logger.info(f"{colored('Coordinates in interval file are invalid', 'red')}")
    logger.info(f'Example: {colored("chr1 100662981 100669420 - L1PA4", "green")}')
    exit(0)
