...
def convert_interval_to_bed(interval_file, logger):
    records_list = []
...
def fasta_format_id(bed_file, fasta_file, logger):
...
def clean_history(hist_id, dataset_id_list, gi, logger):
    start = time.time()
...
def gfetchseq(interval, genome, gkey, up, clean):
    ...
    dataset = upload_bedfile_and_get_dataset(_hist, bed_file, _type, _genome, gi, gurl, gkey, logger)

    dataset_id = dataset.id

    output_id = fetch_sequences_and_get_dataset_id(dataset, gi, logger)

    fasta_file = f'{bed_file[:-4]}.fasta'
    download_fasta(output_id, fasta_file, gi, logger)

    logger.info(colored('Formatting fasta file...', 'blue'))

    upper_fasta = fasta_to_upper(fasta_file, logger, up)
    fasta_format_id(bed_file, upper_fasta, logger)


    id_list = [dataset_id, output_id]

    if clean == 'yes':
        clean_history(_hist, id_list, gi, logger)
